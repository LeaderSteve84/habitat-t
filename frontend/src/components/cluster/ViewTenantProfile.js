...
function ViewTenantProfile() {
  ...
  const endpoint = `/api/tenant/profile/${tenantId}`;
  ...
  const handleDeactivate = async (e, profileId) => {
    e.preventDefault();
    try {
      const response = await axios.delete(`/api/tenant/profile/${profileId}/deactivate`);
  ...
  return (
	...
	<div>
             <Link className="btn btn-warning me-3" as={Link} to={`/cluster_dashboard/${companyId}/${clusterId}/${clusterName}/update_tenant_profile/${tenantId}/${profile.profileId}`}>Update Profile</Link>
                 {" | "}
