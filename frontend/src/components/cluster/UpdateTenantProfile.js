...
const UpdateTenantProfile = () => {
  ...
  const { companyId, clusterId, clusterName, tenantId, profileId } = useParams();
  ...
  const endpoint = `/api/tenant/profile/${profileId}/update`;
  ...
  const handleSubmit = async (e) => {
    ...
    try { 
      ...
      const response = await axios.put(`/api/tenant/profile/${profileId}/update`, data);
      const { msg } = response.data;
      setMessage({
        msg: msg?.toString()
      });
      setTimeout(() => {
        navigate(`/cluster_dashboard/${companyId}/${clusterId}/${clusterName}/tenant_details/${tenantId}/view_tenant_profile/${tenantId}`);
      }, 500);
